...
function displayStory() {
    ...
    console.log("story.words  :   "+story.words)
    console.log("story.inputs  :   "+story.inputs)
    ...
    //reset the word bank 
    $("#bank_words").empty();
    console.log("story.words.length  :  "+story.words.length)
    for (let i = 0; i < story.words.length; i++) {
        console.log("button name in the work bank :    "+story.words[i])
        let html = `<button class="word_bank_button">${story.words[i]}</button>`
        $("#bank_words").append(html)
    }

    //reset the input_filelds (the leftside )
    $("#input_fields").empty();
